...
import bp.demo.iot.operation.manager.repository.dao.pk.TowerCarrierPlatformPolicyRuleKeyDAO;
...
public interface TowerCarrierPlatformPolicyRuleRepository
  ...
  @Query("SELECT * FROM IotOperationSpace.Tower_Carrier_Platform_Policy_Rule " + "WHERE tower_name=?0 AND carrier_name=?1 " + "AND platform_name=?2")
